...
export class CardComponent implements OnInit {
  ...
  get getCardTypeProperties(): any {
    const { ccIcon, ccSingle, cardClasses, mask, expiryDateMask } =
      getCardDetails(this.sel_brand);
    return {
  ...
  get getPatternIndex() {
    const dashIdxs: any[] = [];
    this.getCardTypeProperties.mask
      .split('')
      .forEach((char: any, idx: number) => {
        if (char !== '-') {
          return;
        }
  ...
  get getExpiryDateMask() {
    const index: any[] = [];
    this.getCardTypeProperties.expiryDateMask
      .split('')
      .forEach((char: any, idx: number) => {
        if (char !== '/') {
          return;
  ...
  updateCreditCardInput(val: any, status: boolean) {
    this.cardFormGroup
      .get('cardNumber')!
      .patchValue(val, { emitEvent: status });
  }
  ...
  submitDetails(): void {
    this.requestS.get('http://localhost:3100/api/payment').subscribe((res) => {
